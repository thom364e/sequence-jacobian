...
def hh(Va_p, Vb_p, a_grid, b_grid, z_grid, e_grid, k_grid, beta, eis, rb, ra, chi0, chi1, chi2, Psi1, rborr):
    ...
    b_endo = (c_endo_unc + a_endo_unc + addouter(-z_grid, b_grid, -(1 + ra) * a_grid)
              + get_Psi_and_deriv(a_endo_unc, a_grid, ra, chi0, chi1, chi2)[0]) / (1 + rb_grid[np.newaxis, :, np.newaxis])
    ...
    b_endo = (c_endo_con + a_endo_con
              ...
              + get_Psi_and_deriv(a_endo_con, a_grid, ra, chi0, chi1, chi2)[0]) / (1 + rb_grid[np.newaxis, :, np.newaxis])
    ...
    Va = (1 + ra - Psi2) * uc
    Vb = (1 + rb_grid[np.newaxis, :, np.newaxis]) * uc
