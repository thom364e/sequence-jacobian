...
def make_grids(bmax, hmax, kmax, nB, nH, nK, nZ, rho_z, sigma_z, gamma, qh_lag):
    ...
    h_bhat_grid = grids.agrid(amax=hmax, n=nH, amin = 1e-12)
...
def policy_ss(h_bhat_grid, b_bhat_grid, z_grid, e_grid, k_grid, beta, gamma, theta, sigma, qh, qh_lag, r, alpha, tol=1E-12, max_iter=10_000, debug = False):
    '''
    ...
    for it in range(max_iter):
        Vh_p, Vb_p, h, b, c, _ = hh_housing_v2.hh_housecons_sandbox(Vh_p, Vb_p, h_bhat_grid, b_bhat_grid, 
                                                                    ...
                                                                    sigma, qh, qh_lag, r, alpha, Psi1, debug)
...
def bhat_to_b(h_bhat_grid, b_bhat_grid, b_grid, z_grid, c_bhat, h_bhat, gamma, alpha, r, qh, qh_lag):
    ...
    for j_z in range(nZ):
        for j_h in range(nH):
            ...
            interp_func_c = interp1d(b_endo[j_h, :], c_bhat[j_z, :, j_h], kind='linear', fill_value="extrapolate")
            interp_func_a = interp1d(b_endo[j_h, :], h_bhat[j_z, :, j_h], kind='linear', fill_value="extrapolate")
